...
import { MINISTER, MINISTER_STAFF } from "@/constants/minister";
import Link from "next/link";
import { Copy } from "lucide-react";
import { toast } from "sonner";

const MinisterOffice = () => {
			...
			<div className='container mx-auto pt-12 md:pt-12 pb-12 md:pb-20 px-4 sm:px-6 lg:px-8 relative'>
				...
				<header className='text-center mb-12 md:mb-20'>
					...
					<p className='text-foreground text-base sm:text-lg md:text-xl lg:text-2xl max-w-3xl mx-auto leading-relaxed px-4'>
						Leading the Ministry of Industry, Commerce & Employment towards
				...
				<section className='mb-16 md:mb-28'>
					...
					<div className='group relative max-w-6xl mx-auto mb-6 md:mb-10'>
								...
								<div className='text-center lg:text-left flex-1 w-full'>
									...
									<div className='flex flex-col sm:flex-row gap-2 md:gap-3 justify-center lg:justify-start flex-wrap'>
										<div className='flex items-center gap-2 px-3 sm:px-4 md:px-5 py-2 md:py-3 border-2 rounded-md'>
											...
											<button
												onClick={() => handleCopyEmail(MINISTER.email)}
							...
							<CardFooter className='pt-4 sm:pt-6 md:pt-8 p-0 relative z-10'>
								<div className='w-full space-y-4 md:space-y-6'>
									...
									<div className='space-y-4 md:space-y-6'>
										{/* Appointment */}
										<div>
											<h4 className='text-lg sm:text-xl md:text-2xl font-semibold text-orange-500 uppercase mb-2'>
												Appointment
											</h4>
											<p className='text-foreground text-sm sm:text-base md:text-[18px] leading-relaxed'>
												{MINISTER.biography.appointment}
											</p>
										</div>
										...
										<div>
											...
											<p className='text-foreground text-sm sm:text-base md:text-[18px] leading-relaxed'>
												{MINISTER.biography.constituency}
											</p>
										</div>
										...
										<div>
											...
											<ul className='list-disc list-inside space-y-1 md:space-y-2 text-foreground text-sm sm:text-base md:text-[18px] leading-relaxed'>
												{MINISTER.biography.education.map((edu, index) => (
													<li key={index}>{edu}</li>
												))}
											</ul>
										</div>
										...
										<div>
											...
											<ul className='list-disc list-inside space-y-1 md:space-y-2 text-foreground text-sm sm:text-base md:text-[18px] leading-relaxed'>
												{MINISTER.biography.careerHighlights.map(
													(highlight, index) => (
										...
										<div>
											...
											<ul className='list-disc list-inside space-y-1 md:space-y-2 text-foreground text-sm sm:text-base md:text-[18px] leading-relaxed'>
												{MINISTER.biography.personalInterests.map(
													(interest, index) => (
